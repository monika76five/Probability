...
\cl{A person has $100$ light bulbs whose lifetimes are independent exponentials with mean $5$ hours. If the bulbs are used one at a time, with a failed bulb being replaced immediately by a new one. Suppose that it takes a random time, uniformly distributed over $(0, .5)$, to replace a failed bulb. Approximate the probability that all bulbs have failed by time 550. Note that if $X \sim \textrm{Exponential}(\lambda)$, then $E[X] = \frac{1}{\lambda}$ and $Var(X) = \frac{1}{\lambda^2}$.}
